...
const BlankFillingRender = () => {
    ...
    const onDragEnd = (result) => {
        ...
        if (
            ...
            !isNaN(destination.droppableId)
        ) {
            const tempSolution = [...sol];
            const tempOptions = [...options];
            if (sol[+destination.droppableId].length < 1) {
                tempSolution[+destination.droppableId].push(
                    tempOptions[source.index]
                );
                tempOptions.splice(source.index, 1);
            } else {
                const [removedSol] = tempSolution[
        ...
        if (
            source.droppableId != "option" &&
            destination.droppableId != "option"
        ) {
            const tempSolution = [...sol];
